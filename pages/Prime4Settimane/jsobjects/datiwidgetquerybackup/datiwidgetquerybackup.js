export default {
  ...
  getAllData() {
    ...
    const widget = appsmith.store[widgetName];
    
    if (!widget?.model?.allData) {
      console.warn("Nessun dato disponibile nel widget");
      console.log("Widget disponibile:", widget);
      console.log("Model disponibile:", widget?.model);
      return {};
    }
  ...
  preparaDatiPerQuery() {
    ...
    return datiQuery;
		...
		// In DatiWidgetQuery, aggiungi questa funzione di test
	testConnessione() {
		console.log("=== TEST CONNESSIONE WIDGET ===");
		console.log("Widget disponibili:", Object.keys(appsmith.store || {}));
		console.log("DettaglioSettimanaWidget:", DettaglioSettimanaWidget);
		console.log("getAllData result:", this.getAllData());
		console.log("isWidgetReady:", this.isWidgetReady());
	},
